 ...
 */
@Slf4j
public class Pool2TriggerStrategy implements ReBalanceTriggerStrategy {
    @Override
    public ReBalanceTriggerResult checkRebalance(ReBalanceTriggerContext triggerContext) {
        ...
        if(modelScore.compareTo(new BigDecimal("-2")) < 0) {
            adjt = 30;
        } else if(modelScore.compareTo(new BigDecimal("-2")) >= 0 && modelScore.compareTo(new BigDecimal("0")) <= 0) {
            adjt = 45;
        } else if(modelScore.compareTo(new BigDecimal("0")) > 0 && modelScore.compareTo(new BigDecimal("4")) <= 0){
            adjt = 60;
        }else{
            adjt = 90;
        }
        ...
        Set<String> diffEtfSet = Sets.newHashSet("GLD","PDBC","EWS","VPL","VWO","ILF","VEA","SPY","QQQ","VNQ");
        ...
        triggerResult.setBalTimeDiff(pastDays);
        triggerResult.setEtfDiff(diffEtf);
        triggerResult.setXValue(xValue);
        triggerResult.setAdjt(adjt);
