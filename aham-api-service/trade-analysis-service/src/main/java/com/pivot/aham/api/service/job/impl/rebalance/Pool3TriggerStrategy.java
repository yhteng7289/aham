 ...
 */
@Slf4j
public class Pool3TriggerStrategy implements ReBalanceTriggerStrategy {
    @Override
    public ReBalanceTriggerResult checkRebalance(ReBalanceTriggerContext triggerContext) {
        ...
        if(modelScore.compareTo(new BigDecimal("-2")) < 0) {
            adjt = 30;
        } else if(modelScore.compareTo(new BigDecimal("-2")) >= 0 && modelScore.compareTo(new BigDecimal("6")) <= 0) {
            adjt = 45;
        } else{
            adjt = 60;
        }
        ...
        Set<String> diffEtfSet = Sets.newHashSet("GLD","PDBC","EWS","AAXJ","EWZ","ASHR","RSX","SPY","QQQ","VGK","EWA","EWJ","VNQ");
